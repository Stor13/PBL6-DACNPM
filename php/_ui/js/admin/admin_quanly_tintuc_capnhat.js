...
function validate_form(formData) {
  errorMsg.innerText = "";
  let NotificationID = formData.get("input_NotificationID");
  ...
  if (NotificationID.trim() === "" || Label.trim() === "" || Content.trim() === "" || IsToCourse.trim() === "") {
...
let capnhat_form = document.getElementById("capnhat_tintuc_form");
capnhat_form.addEventListener("submit", function(e) {
  ...
  if (validate_form(formData)) { 
    ...
    xhr.open("POST", "/pbl5/php/views/admin/capnhat_tintuc.php", true);
